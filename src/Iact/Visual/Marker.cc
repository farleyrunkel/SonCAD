...
Marker::Marker(const std::shared_ptr<WorkspaceController>& workspaceController, Styles styles, const MarkerImage& image)
...
Marker::Marker(const std::shared_ptr<WorkspaceController>& workspaceController, Styles styles, const QString& imageName, int size)
    : VisualObject(workspaceController, nullptr)
    , m_styles(styles)
...
Handle(Prs3d_PointAspect) Marker::CreateBitmapPointAspect(
    const MarkerImage& image, Quantity_Color color)
{
...
Handle(Prs3d_PointAspect) Marker::CreateImagePointAspect(const MarkerImage& image)
{
    ...
    {
        return new Prs3d_PointAspect(Aspect_TypeOfMarker::Aspect_TOM_BALL, Colors::Marker, 1.0);
    }
...
Quantity_Color Marker::BackgroundColor() const
{
    return m_colorBg;
}

void Marker::SetBackgroundColor(const Quantity_Color& color)
{
    if(m_colorBg == color)
        return;
    m_colorBg = color;
...
Quantity_Color Marker::Color() const
{
    return m_color;
}

void Marker::SetColor(const Quantity_Color& color)
{
    if(m_color == color)
        return;
    m_color = color;
